...
import com.ibrahim.travelapp.databinding.SearchItemBinding
...
class SearchAdapter() : RecyclerView.Adapter<SearchAdapter.SearchVH>() {
    ...
    inner class SearchVH(val binding: SearchItemBinding) : RecyclerView.ViewHolder(binding.root) {

    }
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): SearchVH {
        return SearchVH(SearchItemBinding.inflate(LayoutInflater.from(parent.context),parent,false))
    }

    override fun onBindViewHolder(holder: SearchVH, position: Int) {
        val travel=allTravelItemList[position]

        Glide.with(holder.binding.searchItemImageview)
            .load(travel?.images?.get(2)?.url.toString())
            .into(holder.binding.searchItemImageview)
